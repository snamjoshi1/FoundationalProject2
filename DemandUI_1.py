...
def calculateDrift(df,fmonths):
    ...
    train_data['Series'] = np.arange(1,len(train_data)+1)
    finaltraindata=train_data[['Series','Month','Year','AvgTemp','AvgHumidity','AvgWindspeed','AvgRainfall','UnemploymentRate','Demand']]
...
def predict():
    ...
    col2.plotly_chart(image)
    calculateDrift(demand_future,fmonths)
